...
function renderListening(){
  ...
  // Cuando se pulsa, reproducir la pregunta actual (en inglés)
  playBtn.onclick = () => {
    // si está en reproducción, cancelamos
    if(listeningUtterance) { stopListeningPlayback(); playBtn.innerText = '🔊 Escuchar en inglés'; playBtn.setAttribute('aria-pressed','false'); return; }
  ...
  // También podemos mostrar la pregunta escrita y las posibles interacciones
  const p = document.createElement('div');
  p.style.marginTop = '12px';
  p.innerText = current.pregunta;
  a.appendChild(p);

  // Opcional: si tiene respuestas (p. ej. escuchar + elegir), renderiza botones de opciones
  if(current.opciones){
    const opts = current.opciones.split('|');
    opts.forEach(o=>{
      const b = document.createElement('button');
      b.innerText = o;
